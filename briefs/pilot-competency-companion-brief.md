...
12. **Dynamic Island top safe-area (device fix):** onboarding screens with a top **step-progress bar** (Intent Quiz, Goal setting, Path Assignment, Profile setup) use a **60px top status-bar inset** (was 43px) so the bar clears the Dynamic Island on modern iPhones; content is trimmed to preserve the 844 height. **Spec for any new screen:** treat the top safe-area inset as **≈59–60pt** on Dynamic Island devices — never place a flush full-width element (bar, chip, header rule) above ~60px from the frame top.
...
## 18. Build, Audit & Change Log (design phase)

> **Purpose:** the complete record of what was actually **built, decided, audited, and fixed** during the OnCourse Aviation design phase, so any stakeholder or downstream agent can see the full picture at a glance. This section is the authoritative "what exists now." Where it differs from earlier proposal-era text, **§18 and §8.0 win.**

### 18.1 What was built — the OnCourse Aviation Figma file

- **File:** *OnCourse Aviation* — `https://www.figma.com/design/VfD60619fgSHWnd3EtFDUg`
- **Pages (4):** 📐 Design System · 📱 App · Core Screens (V0–V2) · 🧭 Later Phase (post-engine) · 📖 Cover & Handoff.
- **Design system (📐):** color **tokens as Figma variables** (`OnCourse/Color`: bg/base, bg/pure-black, bg/surface, bg/surface-elevated, border/subtle, text/primary, text/secondary, brand/teal, brand/primary-deep, status/amber, status/red, status/green) · **Inter type ramp** as text styles (Display · H1 · H2 · H3 · Body · Body-sm · Caption) · **11 component sets** (Button, Status Pill, Selection Chip, Category Chip, Option Card, Toggle, Input Field, Bottom Tab Bar, FAB, Streak Chip, Avatar).
- **Core Screens (38):**
  - *Session/exercise (9):* MCQ · Hydraulics, Flashcard · Limitations, True-False · Performance, Cloze · Memory items, Drag-drop · Abnormal, + answered/checked states for Flashcard, True-False, Cloze, Drag-drop.
  - *Onboarding & identity (10):* 00 Profile setup (aircraft + role), 01 Welcome (Zero-Login), **01b Disclaimer (Before you start)**, 02 Intent Quiz, 03 Quick tour, 04 Goal setting, 05 Path Assignment, 06 First-session choice, 07 Value & Sign-up, Welcome Back (Sign in).
  - *Home & engine (7):* Flight Plan · Home, Flight Plan — Day 0 (cold start), Flight Plan — Offline, Revision Pool, Path Detail, **Why? sheet (over Flight Plan)**, Session Summary.
  - *Train / self-directed (4):* Train, Quick Test, Custom Quiz, **Search · Natural language**.
  - *Progress / Profile / system (5):* Progress, Profile, Settings, AI & Trust Hub, Streak.
  - *Notification surfaces (3):* Lock screen · notification, Home widget, Micro-quiz overlay.
- **Later Phase (4):** Action Currentness, Competency Currentness, Competency Detail · KNO, Admin Review Console.
- **Prototype:** two entry flows (Play-first → Welcome; Returning → Welcome Back) and **54 wired tap connections**; full first-run → daily-loop → tabs coverage. A live, verified **navigation map** of every page-to-page connection was produced as a companion HTML reference.

### 18.2 Direction & positioning decisions (founder-directed, canonical)

1. **Visual language: Light + Teal / Inter** — pivoted from the earlier dark "Glass-Cockpit" proposal (§9.1, now historical) to match the founder's existing *"vector — V0"* reference. Palette + components per the §9 as-built note.
2. **The product is the *method*, not the app** — the core product is the **Competency Continuity Method**; the app is delivery, the method is the licensable/defensible asset. Propagated through §1.1/§1.2/§1.4 and §4.5/§4.6/§4.10/§4.11.
3. **Aircraft/type-agnostic positioning** — the product is never tied to Airbus/A320. A320 is only the *first content set*, applied downstream at profile setup. Narrative copy sweep removed hardcoded "A320" from onboarding (Quick tour, Value & Sign-up) and the Cover subtitle; aircraft mentions remain only as *content* (chips, profile data, category tags, the picker).
4. **As-built founder changes** (also §8.0): home header = gradient avatar + uppercase greeting + **first name only** + A320/streak/settings chips; home surfaces **Quick Test / Custom Quiz** self-directed entries; Custom Quiz reproduces the shipped config layout **+ a new "Suggested for you" hero**; Quick Test reproduces the mode-picker; **profile-setup-first** onboarding; **Welcome Back** returning-user screen.

### 18.3 Audits performed & outcomes (Persona Walkthrough Specialist method)

Cognitive walkthroughs simulating each persona's reactions fold-by-fold (LIFT / Cialdini / Fogg), read-only, then actioned on founder approval.

- **Maya (beachhead cadet) — first-run walkthrough.** Findings F-1…F-5. **Applied:** **F-1 + F-2** — a three-row **trust-proof block** on the Welcome screen surfacing credibility + privacy at first contact (was only implicit at the fold-5 tutorial). Later reframed method-first (§18.2). *(F-3 cold-start home already existed; F-4 First-session primary already had a "RECOMMENDED" badge; F-5 SRS-tooltip noted as future polish.)*
- **David (expert captain) & Sam (career-climber) — comparative walkthrough.** Surfaced three audience conflicts: rank/identity language, hand-holding vs. efficiency, continuity vs. advancement. Findings R-1…R-4. **Applied:** **R-1** rank-neutral path name; **R-2** "Skip the tour — I know the formats" link on Quick tour; **R-3** "Know exactly what you need? Search instead" link on Intent Quiz (→ NL Search). **Deferred by design:** **R-4** career/command-prep is a later-phase persona (Sam) — keep a "coming" breadcrumb, do not re-tune first-run now; role-aware path-name variants (FO/Captain/Cadet) noted as a future enhancement.

### 18.4 Screens added to close the last V1 gaps

- **01b · Disclaimer (Before you start)** *(§43, founder hard requirement §1.3)* — "A companion — never a replacement." + the two hard rules (manuals/SOPs first; qualification stays with the operator) + "lives in Settings → About" + *I understand — continue*. Inserted **Welcome → Disclaimer → Profile setup**.
- **Why? sheet (over Flight Plan)** *(§11)* — bottom sheet: plain-language reason (*next in your path* + *N due*), "How OnCourse decides → AI & Trust Hub", *Pick another due pool*, *Got it — start*. Opens from the hero's "next in your path" chip.
- **Search · Natural language** *(§23)* — "Ask for exactly what you need": search field, suggestion chips, auto-built quiz preview (*8 Q · ≈4 min · sources: FCOM 1.29*), pilot-reviewed note, *Start this quiz*. Reached from the Intent-Quiz "Search instead" link and Train's search entry.

### 18.5 Fixes & QA passes (chronological)

- **Component library + token binding**, **DM Sans → Inter** text styles, **emoji → SVG icons**, **contrast pass**, page reorganization, numbering/Cover cleanup *(earlier remediation)*.
- **Missing V1 states** built: cold-start Flight Plan (Day 0), Offline state, Streak screen.
- **Welcome trust-proof block** (F-1/F-2): *Type-agnostic by design* · *Sourced from official documentation, reviewed by pilots* · *Private by default — never shared with your airline*.
- **R-1 rank-neutral path name**: "Sharp FO — Systems & Abnormals" → **"Sharp — Systems & Abnormals"** across 8 nodes / 7 screens and throughout this brief (also fixed a two-line wrap on the Path Assignment card).
- **R-2 / R-3 expert fast-path links** added to Quick tour and Intent Quiz.
- **Positioning-copy sweep** — hardcoded "A320" removed from narrative onboarding copy; Cover subtitle now reads "Method."
- **Dynamic Island top safe-area fix** — onboarding step-progress bar raised to a **60px top inset** (was 43px) so it clears the island on modern iPhones; content trimmed to preserve 844 height. Applies to Intent Quiz, Goal setting, Path Assignment, Profile setup. **Spec for new screens:** top safe-area ≈ 59–60pt on Dynamic Island devices; never place a flush full-width element above ~60px from the frame top.
- **Full layout QA pass** across all 42 frames (geometry + consistency audits): all screens on-grid, all 390×844, iPhone treatment (46px corners + border) consistent. Fixed the **Streak** description text overflowing its card (334→286, now wraps). Custom Quiz below-fold content confirmed as intended scroll.
- **Prototype wiring pass** — connected every previously-orphaned screen (Value & Sign-up, AI & Trust Hub) and the expert links; later reconciliation caught and fixed the one connection that never applied (**Home active-path strip → Path Detail**).
